...
# -*- coding: utf-8 -*-
"""
경남 HTML 파일들의 메타(description/keywords) 태그를
...
from update_gyeongnam_detail_info import REGION_DISTRICTS as DETAIL_REGION_DATA

THEME_DISPLAY = {slug: info["name"] for slug, info in THEME_INFO.items()}

REGION_META = {
    "changwon": {"display": "경남 창원시", "alias": "창원"},
    "gimhae": {"display": "경남 김해시", "alias": "김해"},
    "jinju": {"display": "경남 진주시", "alias": "진주"},
    "tongyeong": {"display": "경남 통영시", "alias": "통영"},
    "sacheon": {"display": "경남 사천시", "alias": "사천"},
    "miryang": {"display": "경남 밀양시", "alias": "밀양"},
    "yangsan": {"display": "경남 양산시", "alias": "양산"},
    "geoje": {"display": "경남 거제시", "alias": "거제"},
    "changnyeong": {"display": "경남 창녕군", "alias": "창녕"},
    "uiryeong": {"display": "경남 의령군", "alias": "의령"},
    "haman": {"display": "경남 함안군", "alias": "함안"},
    "hapcheon": {"display": "경남 합천군", "alias": "합천"},
    "hamyang": {"display": "경남 함양군", "alias": "함양"},
    "geochang": {"display": "경남 거창군", "alias": "거창"},
    "sancheong": {"display": "경남 산청군", "alias": "산청"},
    "hadong": {"display": "경남 하동군", "alias": "하동"},
    "namhae": {"display": "경남 남해군", "alias": "남해"},
    "goseong": {"display": "경남 고성군", "alias": "고성"},
}

GYEONGNAM_OVERVIEW_REGIONS = [
    "changwon",
    "gimhae",
    "jinju",
    "geoje",
...
def get_region_info(region_slug: str) -> dict:
    if region_slug not in REGION_META:
        raise KeyError(f"지원하지 않는 경남 지역: {region_slug}")
...
def generate_meta_for_gyeongnam() -> Tuple[str, str]:
    pairs = build_region_pairs(GYEONGNAM_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"경남 마사지천국은 {highlight} 등 경남 주요 생활권의 마사지·스파 정보를 한눈에 비교하고 예약할 수 있도록 돕는 플랫폼입니다.",
        f"{highlight} 라인의 경남 마사지샵 정보를 마사지천국에서 손쉽게 확인하고 지역별 힐링 코스를 선택하세요.",
        f"창원·김해·진주에서 거제·통영·양산까지 {highlight} 중심으로 경남 마사지 정보를 큐레이션합니다.",
    ]
    description = deterministic_pick("gyeongnam-overview", variants)
...
def generate_meta_for_gyeongnam_theme(theme_slug: str) -> Tuple[str, str]:
    theme_name = THEME_DISPLAY[theme_slug]
    pairs = build_region_pairs(GYEONGNAM_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"경남 {theme_name} 전문 업체는 {highlight} 등 핵심 생활권에 모여 있으며, 마사지천국에서 비교 예약이 가능합니다.",
        f"{highlight} 중심 경남 {theme_name} 코스를 마사지천국에서 모아 정리했습니다. 테마별 프로그램을 한 번에 비교하세요.",
        f"경남 {theme_name} 정보는 마사지천국에 모두 모였습니다. {highlight} 라인의 인기샵과 후기를 확인해 보세요.",
    ]
    description = deterministic_pick(f"gyeongnam-theme-{theme_slug}", variants)
...
def generate_meta_for_region(region_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 일대에 모여 있는 경남 {info['alias']} 마사지샵을 마사지천국에서 찾아보고 예약하세요. {info['description']}",
        f"경남 {info['alias']} 마사지 정보는 {districts_text} 라인에 집중되어 있으며, 마사지천국에서 인기 힐링 코스를 비교할 수 있습니다. {info['description']}",
        f"{districts_text} 중심으로 형성된 경남 {info['alias']} 마사지·스파 업체를 마사지천국이 한눈에 정리했습니다. {info['description']}",
    ]
...
def generate_meta_for_region_theme(region_slug: str, theme_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 라인의 경남 {info['alias']} {theme_name} 전문샵을 마사지천국에서 비교하고 예약할 수 있습니다.",
        f"{districts_text} 중심으로 형성된 경남 {info['alias']} {theme_name} 프로그램을 마사지천국이 정리했습니다.",
...
def determine_context(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
    name = file_path.name
    if name == "gyeongnam.html":
        return None, None

    if not name.startswith("gyeongnam-") or not name.endswith(".html"):
        return None, None

    slug_part = name[len("gyeongnam-") : -len(".html")]
    if not slug_part:
        return None, None
...
def update_file(file_path: Path) -> bool:
    ...
    if region_slug is None and theme_slug is None:
        description, keywords = generate_meta_for_gyeongnam()
    elif region_slug is None and theme_slug:
        description, keywords = generate_meta_for_gyeongnam_theme(theme_slug)
...
def main() -> None:
    ...
    target_files = list(public_dir.glob("gyeongnam*.html"))
    if not target_files:
        print("처리할 경남 HTML 파일이 없습니다")
        return
    ...
    print(f"\n[완료] 총 {updated_count}개 경남 지역 파일의 메타 태그를 업데이트했습니다.")
