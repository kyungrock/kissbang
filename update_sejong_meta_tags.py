...
# -*- coding: utf-8 -*-
"""
세종 HTML 파일들의 메타(description/keywords) 태그를
읍·면·동 정보에 맞춰 자동 업데이트하는 스크립트
"""
...
from update_sejong_detail_info import REGION_DISTRICTS as DETAIL_REGION_DATA

THEME_DISPLAY = {slug: info["name"] for slug, info in THEME_INFO.items()}

REGION_META = {
    "sejong": {"display": "세종특별자치시", "alias": "세종"},
}

SEJONG_OVERVIEW_REGIONS = ["sejong"]
...
def get_region_info(region_slug: str) -> dict:
    if region_slug not in REGION_META:
        raise KeyError(f"지원하지 않는 세종 지역: {region_slug}")
...
def get_region_districts(region_slug: str, limit: int = 8) -> List[str]:
    info = get_region_info(region_slug)
    return info["districts"][:limit]


def build_region_pairs(region_slugs: Iterable[str], per_region: int = 2) -> List[Tuple[str, str]]:
...
def generate_meta_for_sejong() -> Tuple[str, str]:
    pairs = build_region_pairs(SEJONG_OVERVIEW_REGIONS, per_region=6)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"세종 마사지천국은 {highlight} 등 세종 주요 생활권의 마사지·스파 정보를 한눈에 비교하고 예약할 수 있도록 돕는 플랫폼입니다.",
        f"{highlight} 라인의 세종 마사지샵 정보를 마사지천국에서 손쉽게 확인하고 지역별 힐링 코스를 선택하세요.",
        f"행정중심복합도시의 세종에서는 {highlight} 중심으로 다양한 마사지 코스를 만나볼 수 있습니다.",
    ]
    description = deterministic_pick("sejong-overview", variants)
...
def generate_meta_for_sejong_theme(theme_slug: str) -> Tuple[str, str]:
    theme_name = THEME_DISPLAY[theme_slug]
    pairs = build_region_pairs(SEJONG_OVERVIEW_REGIONS, per_region=6)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"세종 {theme_name} 전문 업체는 {highlight} 등 주요 생활권에 모여 있으며, 마사지천국에서 비교 예약이 가능합니다.",
        f"{highlight} 중심 세종 {theme_name} 코스를 마사지천국이 모아 정리했습니다. 테마별 프로그램을 한 번에 비교하세요.",
        f"세종 {theme_name} 정보는 마사지천국에 모두 모였습니다. {highlight} 라인의 인기샵과 후기를 확인해 보세요.",
    ]
    description = deterministic_pick(f"sejong-theme-{theme_slug}", variants)
...
def generate_meta_for_region(region_slug: str) -> Tuple[str, str]:
    info = get_region_info(region_slug)
    districts = get_region_districts(region_slug, limit=8)
    districts_text = ", ".join(districts)

    variants = [
        f"{info['alias']} {districts_text} 일대에 모여 있는 세종 {info['alias']} 마사지샵을 마사지천국에서 찾아보고 예약하세요. {info['description']}",
        f"세종 {info['alias']} 마사지 정보는 {districts_text} 라인에 집중되어 있으며, 마사지천국에서 인기 힐링 코스를 비교할 수 있습니다. {info['description']}",
        f"{districts_text} 중심으로 형성된 세종 {info['alias']} 마사지·스파 업체를 마사지천국이 한눈에 정리했습니다. {info['description']}",
    ]
...
def generate_meta_for_region_theme(region_slug: str, theme_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 라인의 세종 {info['alias']} {theme_name} 전문샵을 마사지천국에서 비교하고 예약할 수 있습니다.",
        f"{districts_text} 중심으로 형성된 세종 {info['alias']} {theme_name} 프로그램을 마사지천국이 정리했습니다.",
...
def determine_context(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not name.startswith("sejong-") or not name.endswith(".html"):
        return None, None

    slug_part = name[len("sejong-") : -len(".html")]
    if not slug_part:
        return None, None
...
def update_file(file_path: Path) -> bool:
    ...
    if region_slug is None and theme_slug is None:
        description, keywords = generate_meta_for_sejong()
    elif region_slug is None and theme_slug:
        description, keywords = generate_meta_for_sejong_theme(theme_slug)
...
def main() -> None:
    ...
    target_files = list(public_dir.glob("sejong*.html"))
    if not target_files:
        print("처리할 세종 HTML 파일이 없습니다.")
        return
    ...
    print(f"\n[완료] 총 {updated_count}개 세종 지역 파일의 메타 태그를 업데이트했습니다.")
