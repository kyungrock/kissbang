...
# -*- coding: utf-8 -*-
"""
모든 지역 HTML 파일의 관련정보 버튼 위치를 일괄 수정
...
def fix_related_info_button(file_path: Path) -> bool:
    """파일의 관련정보 버튼 위치 수정"""
    try:
        content = file_path.read_text(encoding='utf-8')
        original = content
        
        # 잘못된 패턴 찾기: type-filter-dropdown 안에 관련정보 버튼이 있는 경우
...
def main():
    ...
    for file_path in sorted(html_files):
        ...
        try:
            if fix_related_info_button(file_path):
                updated_count += 1
                print("✅ 완료")
            else:
                skipped_count += 1
