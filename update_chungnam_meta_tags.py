...
# -*- coding: utf-8 -*-
"""
충남 HTML 파일들의 메타(description/keywords) 태그를
...
from update_chungnam_detail_info import REGION_DISTRICTS as DETAIL_REGION_DATA
...
# 충남 지역별 표시 정보
REGION_META = {
    "cheonan": {"display": "충남 천안시", "alias": "천안"},
    "asan": {"display": "충남 아산시", "alias": "아산"},
    "gongju": {"display": "충남 공주시", "alias": "공주"},
    "boryeong": {"display": "충남 보령시", "alias": "보령"},
    "seosan": {"display": "충남 서산시", "alias": "서산"},
    "dangjin": {"display": "충남 당진시", "alias": "당진"},
    "nonsan": {"display": "충남 논산시", "alias": "논산"},
    "gyeryong": {"display": "충남 계룡시", "alias": "계룡"},
    "geumsan": {"display": "충남 금산군", "alias": "금산"},
    "buyeo": {"display": "충남 부여군", "alias": "부여"},
    "seocheon": {"display": "충남 서천군", "alias": "서천"},
    "yesan": {"display": "충남 예산군", "alias": "예산"},
    "hongseong": {"display": "충남 홍성군", "alias": "홍성"},
    "cheongyang": {"display": "충남 청양군", "alias": "청양"},
    "taean": {"display": "충남 태안군", "alias": "태안"},
}

CHUNGNAM_OVERVIEW_REGIONS = [
    "cheonan",
    "asan",
    "seosan",
    "dangjin",
...
def get_region_info(region_slug: str) -> dict:
    if region_slug not in REGION_META:
        raise KeyError(f"지원하지 않는 충남 지역: {region_slug}")
...
def generate_meta_for_chungnam() -> Tuple[str, str]:
    pairs = build_region_pairs(CHUNGNAM_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"충남 마사지천국은 {highlight} 등 충남 주요 생활권의 마사지·스파 정보를 한눈에 비교하고 예약할 수 있도록 돕는 플랫폼입니다.",
        f"{highlight} 라인의 충남 마사지샵 정보를 마사지천국에서 손쉽게 확인하고 지역별 힐링 코스를 선택하세요.",
        f"천안·아산·서산부터 예산·홍성까지 {highlight} 중심으로 충남 마사지 정보를 큐레이션합니다.",
    ]
    description = deterministic_pick("chungnam-overview", variants)
...
def generate_meta_for_chungnam_theme(theme_slug: str) -> Tuple[str, str]:
    theme_name = THEME_DISPLAY[theme_slug]
    pairs = build_region_pairs(CHUNGNAM_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"충남 {theme_name} 전문 업체는 {highlight} 등 핵심 생활권에 모여 있으며, 마사지천국에서 비교 예약이 가능합니다.",
        f"{highlight} 중심 충남 {theme_name} 코스를 마사지천국에서 모아 보고 원하는 프로그램을 선택하세요.",
        f"충남 {theme_name} 정보는 마사지천국에서 확인하세요. {highlight} 라인의 인기샵과 후기를 한 번에 비교할 수 있습니다.",
    ]
    description = deterministic_pick(f"chungnam-theme-{theme_slug}", variants)
...
def generate_meta_for_region(region_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"충남 {info['alias']} 마사지 정보는 {districts_text} 일대에 집중되어 있으며, 마사지천국에서 인기 힐링 코스를 비교할 수 있습니다. {info['description']}",
        f"{info['alias']} {districts_text} 라인의 마사지·스파 업체를 마사지천국이 한눈에 정리했습니다. {info['description']}",
        f"{districts_text} 중심으로 모여 있는 충남 {info['alias']} 마사지샵을 마사지천국에서 찾아보고 예약하세요. {info['description']}",
    ]
...
def generate_meta_for_region_theme(region_slug: str, theme_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"충남 {info['alias']} {theme_name} 라인은 {districts_text}을 중심으로 형성되어 있으며, 마사지천국에서 테마별 전문샵을 비교하고 예약할 수 있습니다.",
        f"{districts_text} 중심 충남 {info['alias']} {theme_name} 프로그램을 마사지천국이 정리했습니다.",
...
def determine_context(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
    name = file_path.name
    if name == "chungnam.html":
        return None, None

    if not name.startswith("chungnam-") or not name.endswith(".html"):
        return None, None

    slug_part = name[len("chungnam-") : -len(".html")]
    if not slug_part:
        return None, None
...
def update_file(file_path: Path) -> bool:
    ...
    if region_slug is None and theme_slug is None:
        description, keywords = generate_meta_for_chungnam()
    elif region_slug is None and theme_slug:
        description, keywords = generate_meta_for_chungnam_theme(theme_slug)
...
def main() -> None:
    ...
    target_files = list(public_dir.glob("chungnam*.html"))
    if not target_files:
        print("처리할 충남 HTML 파일이 없습니다.")
        return
    ...
    print(f"\n[완료] 총 {updated_count}개 충남 지역 파일의 메타 태그를 업데이트했습니다.")
