...
# -*- coding: utf-8 -*-
"""
대전 HTML 파일들의 메타(description/keywords) 태그를
구·동 정보에 맞춰 자동 업데이트하는 스크립트
"""
...
from update_daejeon_detail_info import REGION_DISTRICTS as DETAIL_REGION_DATA
...
# 대전 5개 자치구 정보
REGION_META = {
    "donggu": {"display": "대전 동구", "alias": "동구"},
    "junggu": {"display": "대전 중구", "alias": "중구"},
    "seogu": {"display": "대전 서구", "alias": "서구"},
    "yuseong": {"display": "대전 유성구", "alias": "유성구"},
    "daedeok": {"display": "대전 대덕구", "alias": "대덕구"},
}

DAEJEON_OVERVIEW_REGIONS = ["seogu", "yuseong", "donggu", "junggu", "daedeok"]
...
def get_region_info(region_slug: str) -> dict:
    if region_slug not in REGION_META:
        raise KeyError(f"지원하지 않는 대전 구: {region_slug}")
...
def generate_meta_for_daejeon() -> Tuple[str, str]:
    pairs = build_region_pairs(DAEJEON_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"대전 마사지천국은 {highlight} 등 대전 주요 생활권의 마사지·스파 정보를 한눈에 비교하고 예약할 수 있도록 돕는 플랫폼입니다.",
        f"{highlight}를 포함한 대전 전역 마사지샵 정보를 마사지천국에서 손쉽게 확인하고 지역별 힐링 코스를 선택하세요.",
        f"서구 둔산과 유성온천, 동구/중구 원도심, 대덕 벤처밸리까지 {highlight} 중심으로 대전 마사지 정보를 큐레이션합니다.",
    ]
    description = deterministic_pick("daejeon-overview", variants)
...
def generate_meta_for_daejeon_theme(theme_slug: str) -> Tuple[str, str]:
    theme_name = THEME_DISPLAY[theme_slug]
    pairs = build_region_pairs(DAEJEON_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"대전 {theme_name} 전문 업체는 {highlight} 등 핵심 생활권에 모여 있으며, 마사지천국에서 비교 예약이 가능합니다.",
        f"{highlight} 중심 대전 {theme_name} 코스를 마사지천국이 모아 정리했습니다. 테마별 프로그램을 한 번에 비교하세요.",
        f"대전 {theme_name} 정보는 마사지천국에 모두 모였습니다. {highlight} 라인의 인기샵과 후기를 확인해 보세요.",
    ]
    description = deterministic_pick(f"daejeon-theme-{theme_slug}", variants)
...
def generate_meta_for_region(region_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 일대에 모여 있는 대전 {info['alias']} 마사지샵을 마사지천국에서 찾아보고 예약하세요. {info['description']}",
        f"대전 {info['alias']} 마사지 정보는 {districts_text} 라인에 집중되어 있으며, 마사지천국에서 인기 힐링 코스를 비교할 수 있습니다. {info['description']}",
        f"{districts_text} 중심으로 형성된 대전 {info['alias']} 마사지·스파 업체를 마사지천국이 한눈에 정리했습니다. {info['description']}",
    ]
...
def generate_meta_for_region_theme(region_slug: str, theme_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 라인의 대전 {info['alias']} {theme_name} 전문샵을 마사지천국에서 비교하고 예약할 수 있습니다.",
        f"{districts_text} 중심으로 형성된 대전 {info['alias']} {theme_name} 프로그램을 마사지천국이 정리했습니다.",
...
def determine_context(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
    name = file_path.name
    if name == "daejeon.html":
        return None, None

    if not name.startswith("daejeon-") or not name.endswith(".html"):
        return None, None

    slug_part = name[len("daejeon-") : -len(".html")]
    if not slug_part:
        return None, None
...
def update_file(file_path: Path) -> bool:
    ...
    if region_slug is None and theme_slug is None:
        description, keywords = generate_meta_for_daejeon()
    elif region_slug is None and theme_slug:
        description, keywords = generate_meta_for_daejeon_theme(theme_slug)
...
def main() -> None:
    ...
    target_files = list(public_dir.glob("daejeon*.html"))
    if not target_files:
        print("처리할 대전 HTML 파일이 없습니다.")
        return
    ...
    print(f"\n[완료] 총 {updated_count}개 대전 지역 파일의 메타 태그를 업데이트했습니다.")
