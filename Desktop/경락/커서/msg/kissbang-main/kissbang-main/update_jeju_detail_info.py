...
# -*- coding: utf-8 -*-
"""
제주 HTML 파일들의 관련정보 모달 내용을 자동으로 업데이트하는 스크립트
"""
...
REGION_DISTRICTS = {
    'si': {
        'name': '제주시',
        'districts': [
            '시내 중심', '연동', '노형동', '이도동', '삼도동', '용담동', '건입동'
        ],
        'description': '제주도의 북부 중심 도시로, 제주공항과 가까워 접근성이 뛰어납니다.'
    },
    'seogwipo': {
        'name': '서귀포',
        'districts': [
            '시내 중심', '서귀동', '중문동', '대정읍', '안덕면', '남원읍', '성산읍'
        ],
        'description': '제주도의 남부 중심 도시로, 중문관광단지와 한라산이 있는 관광 도시입니다.'
    },
}
...
TOURISM_SPOTS = [
    {
        'category': '자연 관광지',
        'spots': [
            '한라산 국립공원: 제주도의 대표 산으로, 등산과 자연 감상을 즐길 수 있습니다.',
            '성산일출봉: 제주도의 동쪽 끝에 위치한 화산체로, 일출 명소입니다.',
            '천지연폭포: 서귀포에 위치한 아름다운 폭포로, 자연 감상을 즐길 수 있습니다.',
            '정방폭포: 서귀포에 위치한 해안 폭포로, 독특한 경관을 자랑합니다.'
        ]
    },
    {
        'category': '해안 휴양지',
        'spots': [
            '협재해수욕장: 제주도의 서쪽 해안에 위치한 해수욕장으로, 아름다운 해안을 자랑합니다.',
            '함덕해수욕장: 제주시 동쪽 해안에 위치한 해수욕장으로, 조용한 분위기를 즐길 수 있습니다.',
            '중문관광단지: 서귀포에 위치한 대형 관광단지로, 다양한 엔터테인먼트 시설이 있습니다.',
            '카멜리아힐: 제주도의 동쪽에 위치한 동백나무 테마파크로, 아름다운 정원을 자랑합니다.'
        ]
    },
    {
        'category': '문화 유적지',
        'spots': [
            '제주 올레길: 제주도를 둘러싼 걷기 코스로, 자연과 문화를 함께 즐길 수 있습니다.',
            '성읍민속마을: 제주도의 전통 마을로, 제주 전통 문화를 체험할 수 있습니다.',
            '제주 박물관: 제주도의 역사와 문화를 감상할 수 있는 박물관입니다.',
            '용두암: 제주시에 위치한 해안 명소로, 용의 머리 모양을 한 바위입니다.'
        ]
    }
]
...
FOOD_RECOMMENDATIONS = [
    {
        'category': '제주 해산물 전문 음식점',
        'foods': [
            '제주 회: 제주도의 대표 음식으로, 신선한 회를 맛볼 수 있습니다.',
            '제주 전복: 제주도에서 잡은 신선한 전복 요리를 즐길 수 있습니다.',
            '제주 갈치조림: 제주도의 대표 음식으로, 진한 국물이 일품입니다.',
            '제주 해산물: 제주도에서 잡은 신선한 해산물을 즐길 수 있습니다.'
        ]
    },
    {
        'category': '제주 전통 음식',
        'foods': [
            '제주 흑돼지: 제주도의 대표 음식으로, 특별한 맛을 자랑합니다.',
            '제주 한정식: 제주도의 전통 한정식을 맛볼 수 있습니다.',
            '제주 갈치조림: 제주도의 대표 음식으로, 진한 국물이 일품입니다.',
            '제주 올레길 음식: 제주 올레길을 따라 즐길 수 있는 다양한 음식입니다.'
        ]
    },
    {
        'category': '제주 카페 및 디저트',
        'foods': [
            '제주 카페 거리: 제주도의 다양한 카페에서 커피와 디저트를 즐길 수 있습니다.',
            '제주 해안가 카페: 제주도 해안을 조망하며 커피를 마실 수 있는 카페입니다.',
            '제주 전통찻집: 제주도의 전통 차와 한과를 즐길 수 있는 찻집입니다.',
            '제주 디저트 카페: 제주도의 다양한 디저트와 브런치를 즐길 수 있는 카페입니다.'
        ]
    }
]
...
def generate_introduction(region_name, theme_name=None):
    """소개글 생성"""
    if region_name == '제주':
        region_description = '한국의 대표 관광지로, 한라산과 아름다운 해안이 있는 섬입니다.'
        display_name = ''
        title_prefix = '제주'
    else:
        region_key = {
        ...
        region_description = region_info.get('description', '제주 지역')
        display_name = region_name
        title_prefix = f'제주 {region_name}'
    ...
    if theme_name:
        ...
        return f"""            <h3>{theme_info['icon']} {title_prefix} {theme_info['name']} 소개</h3>
            ...
            <p>
              {title_prefix} {theme_info['name']}는 단순한 피로 회복을 넘어서 몸과 마음의 완전한 힐링을 추구합니다. 전문 관리사들이 제공하는 다양한 {theme_info['name']} 기법을 통해 일상의 스트레스와 피로를 완전히 해소하고, {display_name if display_name else '제주'}의 여유로운 분위기 속에서 진정한 휴식을 경험할 수 있습니다.
            </p>"""
    else:
        return f"""            <h3>🏝️ {title_prefix} 마사지 소개</h3>
            <p>
              <strong>{title_prefix} 마사지</strong>는 {region_description} {display_name if display_name else '제주'}에서 제공되는 전문 마사지 서비스입니다. {display_name if display_name else '제주'}은 다양한 지역에 마사지 업체들이 위치해 있습니다.
            </p>
            <p>
              {title_prefix} 마사지는 단순한 피로 회복을 넘어서 몸과 마음의 완전한 힐링을 추구합니다. 전문 관리사들이 제공하는 다양한 마사지 기법을 통해 일상의 스트레스와 피로를 완전히 해소하고, {display_name if display_name else '제주'}의 여유로운 분위기 속에서 진정한 휴식을 경험할 수 있습니다.
            </p>"""
...
def generate_features(region_name, theme_name=None):
    ...
    if theme_name:
        ...
        return f"""            <h4>✨ {title_prefix} {theme_info['name']}의 특징과 내용</h4>
            <p>
              <strong>{title_prefix} {theme_info['name']}</strong>는 {display_name if display_name else '제주'}만의 독특한 특성을 가지고 있습니다. 전문 {theme_info['name']} 서비스를 제공하며, 고객의 만족도를 최우선으로 생각합니다. {display_name if display_name else '제주'}의 지역적 특색과 {theme_info['name']}의 전문성이 결합되어 최고의 힐링 경험을 선사합니다.
            </p>
...
{features_html}
        ...
        return f"""            <h4>✨ {title_prefix} 마사지의 특징과 내용</h4>
            <p>
              <strong>{title_prefix} 마사지</strong>는 {display_name if display_name else '제주'}만의 독특한 특성을 가지고 있습니다. 다양한 마사지 서비스를 제공하며, 고객의 만족도를 최우선으로 생각합니다. {display_name if display_name else '제주'}의 지역적 특색과 전문 마사지 기술이 결합되어 최고의 힐링 경험을 선사합니다.
            </p>
            <div class="info-highlight">
              <h4>🌟 마사지의 핵심 특징</h4>
              <ul>
...
def generate_districts(region_name, theme_name=None):
    ...
    if region_name == '제주':
        # 제주 전체인 경우 시 정보 제공
        districts = ['제주시', '서귀포시']
        districts_html = '\n'.join([f'                <li><strong>{district}:</strong> 제주 {district}에는 전문 마사지 업체들이 위치해 있으며, 각 시별로 특색 있는 마사지 서비스를 제공합니다.</li>' for district in districts])
        theme_display = THEME_INFO[theme_name]['name'] if theme_name and theme_name in THEME_INFO else "마사지"
        return f"""            <h4>🗺️ 제주 {theme_display} 추천 행정구간</h4>
            <p>
              <strong>제주</strong>는 2개 시로 구성되어 있으며, 각 시마다 전문 {theme_display} 업체들이 운영되고 있습니다. 제주의 주요 시별 {theme_display} 특색을 소개합니다.
            </p>
            <div class="info-highlight">
              <h4>🏛️ 제주 주요 시</h4>
              <ul>
{districts_html}
        ...
        theme_display = THEME_INFO[theme_name]['name'] if theme_name and theme_name in THEME_INFO else "마사지"
        return f"""            <h4>🗺️ 제주 {region_name} {theme_display} 추천 행정구간</h4>
            <p>
...
def generate_tourism(region_name, theme_name=None):
    """관광 및 놀이 생성"""
    display_name = '제주' if region_name == '제주' else region_name
    title_prefix = '제주' if region_name == '제주' else f'제주 {region_name}'
...
"""
    ...
    if theme_name:
        ...
        return f"""            <h4>🎨 {title_prefix} {theme_info['name']} 후 추천 관광지 및 놀이</h4>
            <p>
              <strong>{title_prefix} {theme_info['name']}</strong>를 받은 후에는 제주의 다양한 관광지와 놀이 시설을 즐기실 수 있습니다. {theme_info['name']}로 풀린 몸과 마음으로 제주의 특별한 경험을 더욱 풍성하게 만들어보세요.
            </p>
...
{tourism_html.rstrip()}"""
        ...
        return f"""            <h4>🎨 {title_prefix} 마사지 후 추천 관광지 및 놀이</h4>
            <p>
              <strong>{title_prefix} 마사지</strong>를 받은 후에는 제주의 다양한 관광지와 놀이 시설을 즐기실 수 있습니다. 마사지로 풀린 몸과 마음으로 제주의 특별한 경험을 더욱 풍성하게 만들어보세요.
            </p>
...
"""
    ...
    if theme_name:
        ...
        return f"""            <h4>🍽️ {title_prefix} {theme_info['name']} 후 추천 음식 및 경로</h4>
            <p>
              <strong>{title_prefix} {theme_info['name']}</strong>를 받은 후에는 제주의 맛있는 음식으로 에너지를 보충하세요. {theme_info['name']}로 활력을 회복한 후 제주의 특별한 맛을 즐기시면 더욱 완벽한 하루가 됩니다.
            </p>
...
{food_html.rstrip()}"""
        ...
        return f"""            <h4>🍽️ {title_prefix} 마사지 후 추천 음식 및 경로</h4>
            <p>
              <strong>{title_prefix} 마사지</strong>를 받은 후에는 제주의 맛있는 음식으로 에너지를 보충하세요. 마사지로 활력을 회복한 후 제주의 특별한 맛을 즐기시면 더욱 완벽한 하루가 됩니다.
            </p>
...
def generate_services(region_name, theme_name=None):
    ...
    if theme_name:
        ...
        return f"""            <h4>🎁 {title_prefix} {theme_info['name']} 서비스 소개</h4>
            <p>
              <strong>{title_prefix} {theme_info['name']} 업체</strong>에서는 {display_name if display_name else '제주'}만의 특별한 {theme_info['name']} 서비스와 상품을 제공하여 고객님들의 만족도를 극대화하고 있습니다.
            </p>
...
{services_detail_html}
        ...
        return f"""            <h4>🎁 {title_prefix} 마사지 서비스 소개</h4>
            <p>
              <strong>{title_prefix} 마사지 업체</strong>에서는 {display_name if display_name else '제주'}만의 특별한 마사지 서비스와 상품을 제공하여 고객님들의 만족도를 극대화하고 있습니다.
            </p>
            <div class="service-categories">
...
def main():
    ...
    # 1. 제주 전체 파일 업데이트
    print("=== 제주 전체 파일 업데이트 시작 ===")
    file_path = public_dir / 'jeju.html'
    if file_path.exists():
        if update_file(file_path, '제주', None):
            updated_count += 1
    
    # 2. 제주 + 필터테마 파일들 업데이트
    print("\n=== 제주 + 필터테마 파일 업데이트 시작 ===")
    for theme in themes:
        file_path = public_dir / f'jeju-{theme}.html'
        if file_path.exists():
            if update_file(file_path, '제주', theme):
                updated_count += 1
    
    # 3. 제주 + 상세지역 파일들 업데이트
    print("\n=== 제주 + 상세지역 파일 업데이트 시작 ===")
    for region in detail_regions:
        file_path = public_dir / f'jeju-{region}.html'
        if file_path.exists():
            region_name = region_name_map.get(region, region)
            if update_file(file_path, region_name, None):
                updated_count += 1
    
    # 4. 제주 + 상세지역 + 테마 파일들 업데이트
    print("\n=== 제주 + 상세지역 + 테마 파일 업데이트 시작 ===")
    for region in detail_regions:
        for theme in themes:
            file_path = public_dir / f'jeju-{region}-{theme}.html'
            if file_path.exists():
                region_name = region_name_map.get(region, region)
