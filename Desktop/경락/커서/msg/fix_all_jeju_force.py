...
def fix_file_force(text: str) -> str:
    """파일 내용을 강제로 수정 - 모든 패턴 처리"""
    
    # 1단계: 여분의 </div> 제거 - 모든 공백 패턴 찾아서 제거
    # 정규식으로 </div> 다음에 공백 8칸 이상이 있고 그 다음 </div>가 있는 경우
    ...
    for path in sorted(ROOT.glob("jeju*.html")):
        try:
            ...
            if new_text != text:
                ...
                if "</div>                        </div>" in text or "</div>        </div>" in text or re.search(r'</div>\s{8,}</div>', text):
                    # 더 강력하게 수정
                    fixed = text
