...
def fix_file_absolute(text: str) -> str:
    """파일 내용을 절대적으로 수정 - 모든 패턴 처리"""
    ...
    for path in sorted(ROOT.glob("jeju*.html")):
        try:
            ...
            has_extra = (
                "</div>                        </div>" in text or 
                "</div>        </div>" in text or
                bool(re.search(r'</div>\s{8,}</div>', text))
            )
            
            # 수정 시도
            new_text = fix_file_absolute(text)
