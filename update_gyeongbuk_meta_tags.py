...
# -*- coding: utf-8 -*-
"""
경북 HTML 파일들의 메타(description/keywords) 태그를
시·군·읍·면 정보에 맞춰 자동 업데이트하는 스크립트
"""
...
from update_gyeongbuk_detail_info import REGION_DISTRICTS as DETAIL_REGION_DATA

THEME_DISPLAY = {slug: info["name"] for slug, info in THEME_INFO.items()}

REGION_META = {
    "pohang": {"display": "경북 포항시", "alias": "포항"},
    "gyeongju": {"display": "경북 경주시", "alias": "경주"},
    "gimcheon": {"display": "경북 김천시", "alias": "김천"},
    "andong": {"display": "경북 안동시", "alias": "안동"},
    "gumi": {"display": "경북 구미시", "alias": "구미"},
    "yeongju": {"display": "경북 영주시", "alias": "영주"},
    "yeongcheon": {"display": "경북 영천시", "alias": "영천"},
    "gyeongsan": {"display": "경북 경산시", "alias": "경산"},
    "sangju": {"display": "경북 상주시", "alias": "상주"},
    "mungyeong": {"display": "경북 문경시", "alias": "문경"},
    "yeongdeok": {"display": "경북 영덕군", "alias": "영덕"},
    "yeongyang": {"display": "경북 영양군", "alias": "영양"},
    "cheongdo": {"display": "경북 청도군", "alias": "청도"},
    "cheongsong": {"display": "경북 청송군", "alias": "청송"},
    "gunwi": {"display": "경북 군위군", "alias": "군위"},
    "uiseong": {"display": "경북 의성군", "alias": "의성"},
    "bonghwa": {"display": "경북 봉화군", "alias": "봉화"},
    "yecheon": {"display": "경북 예천군", "alias": "예천"},
    "goryeong": {"display": "경북 고령군", "alias": "고령"},
    "seongju": {"display": "경북 성주군", "alias": "성주"},
    "chilgok": {"display": "경북 칠곡군", "alias": "칠곡"},
    "uljin": {"display": "경북 울진군", "alias": "울진"},
    "ulleung": {"display": "경북 울릉군", "alias": "울릉"},
}

GYEONGBUK_OVERVIEW_REGIONS = [
    "pohang",
    "gyeongju",
    "gumi",
    "gimcheon",
...
def get_region_info(region_slug: str) -> dict:
    if region_slug not in REGION_META:
        raise KeyError(f"지원하지 않는 경북 지역: {region_slug}")
...
def generate_meta_for_gyeongbuk() -> Tuple[str, str]:
    pairs = build_region_pairs(GYEONGBUK_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"경북 마사지천국은 {highlight} 등 경북 주요 생활권의 마사지·스파 정보를 한눈에 비교하고 예약할 수 있도록 돕는 플랫폼입니다.",
        f"{highlight} 라인의 경북 마사지샵 정보를 마사지천국에서 손쉽게 확인하고 지역별 힐링 코스를 선택하세요.",
        f"포항·경주·구미에서 안동·영천까지 {highlight} 중심으로 경북 마사지 정보를 큐레이션합니다.",
    ]
    description = deterministic_pick("gyeongbuk-overview", variants)
...
def generate_meta_for_gyeongbuk_theme(theme_slug: str) -> Tuple[str, str]:
    theme_name = THEME_DISPLAY[theme_slug]
    pairs = build_region_pairs(GYEONGBUK_OVERVIEW_REGIONS, per_region=1)
    highlight = ", ".join(f"{display} {district}" for display, district in pairs[:6])

    variants = [
        f"경북 {theme_name} 전문 업체는 {highlight} 등 핵심 생활권에 모여 있으며, 마사지천국에서 비교 예약이 가능합니다.",
        f"{highlight} 중심 경북 {theme_name} 코스를 마사지천국에서 모아 정리했습니다. 테마별 프로그램을 한 번에 비교하세요.",
        f"경북 {theme_name} 정보는 마사지천국에 모두 모였습니다. {highlight} 라인의 인기샵과 후기를 확인해 보세요.",
    ]
    description = deterministic_pick(f"gyeongbuk-theme-{theme_slug}", variants)
...
def generate_meta_for_region(region_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 일대에 모여 있는 경북 {info['alias']} 마사지샵을 마사지천국에서 찾아보고 예약하세요. {info['description']}",
        f"경북 {info['alias']} 마사지 정보는 {districts_text} 라인에 집중되어 있으며, 마사지천국에서 인기 힐링 코스를 비교할 수 있습니다. {info['description']}",
        f"{districts_text} 중심으로 형성된 경북 {info['alias']} 마사지·스파 업체를 마사지천국이 한눈에 정리했습니다. {info['description']}",
    ]
...
def generate_meta_for_region_theme(region_slug: str, theme_slug: str) -> Tuple[str, str]:
    ...
    variants = [
        f"{info['alias']} {districts_text} 라인의 경북 {info['alias']} {theme_name} 전문샵을 마사지천국에서 비교하고 예약할 수 있습니다.",
        f"{districts_text} 중심으로 형성된 경북 {info['alias']} {theme_name} 프로그램을 마사지천국이 정리했습니다.",
...
def determine_context(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
    name = file_path.name
    if name == "gyeongbuk.html":
        return None, None

    if not name.startswith("gyeongbuk-") or not name.endswith(".html"):
        return None, None

    slug_part = name[len("gyeongbuk-") : -len(".html")]
    if not slug_part:
        return None, None
...
def update_file(file_path: Path) -> bool:
    ...
    if region_slug is None and theme_slug is None:
        description, keywords = generate_meta_for_gyeongbuk()
    elif region_slug is None and theme_slug:
        description, keywords = generate_meta_for_gyeongbuk_theme(theme_slug)
...
def main() -> None:
    ...
    target_files = list(public_dir.glob("gyeongbuk*.html"))
    if not target_files:
        print("처리할 경북 HTML 파일이 없습니다.")
        return
    ...
    print(f"\n[완료] 총 {updated_count}개 경북 지역 파일의 메타 태그를 업데이트했습니다.")
